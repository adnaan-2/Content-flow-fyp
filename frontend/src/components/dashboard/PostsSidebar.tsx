...
export default function PostsSidebar() {
  ...
  if (loading) {
    return (
      <Card className="w-80 bg-card dark:bg-card border-border dark:border-border shadow-sm">
        <CardHeader>
          ...
          {posts.length === 0 ? (
            ...
            <div className="p-4 space-y-3">
              {posts.map((post) => (
                <div 
                  key={post._id} 
                  className={`border border-border dark:border-border rounded-lg p-3 transition-all duration-200 bg-card dark:bg-card ${
                    post.status === 'published'
                      ? 'cursor-pointer hover:shadow-md hover:bg-accent/50 dark:hover:bg-accent/50 hover:scale-[1.02] transform hover:border-accent dark:hover:border-accent' 
                ...
                >
                  ...
                  {post.status === 'published' && post.analytics && (
                    <div className="flex gap-4 text-xs text-muted-foreground mb-2 p-2 bg-muted/50 dark:bg-muted/50 rounded border border-border dark:border-border">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {isPostClickable(post) && (
                      <span className="text-xs text-primary bg-primary/10 dark:bg-primary/10 px-2 py-1 rounded border border-primary/20 dark:border-primary/20">
                        Click to view
                      </span>
                    )}
                  </div>
